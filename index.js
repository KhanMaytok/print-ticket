...
app.post('/ticket/invoice/20608151771', (req, res) => { // ANGEL DIVINO BUS - 20608151771
    ...
    printer.printImage(logo).then(function (done) {
        ...
        printer.println(`PUNTO DE EMISIÓN: ${body.seller_agency}`)
        printer.println(`R.U.C. ${body.enterprise_ruc}`);
	    printer.println(`Venta internet autorizados Chiclayo: 958842029 |954909021| 942057662| 907758392`);
        printer.println(printLines());
        ...
        const forma_pago = body.payment_type.toUpperCase() === 'EFECTIVO' ? 'CONTADO' :  body.payment_type;
        printer.println(`FORMA DE PAGO: ${forma_pago}`);
        printer.println(`NRO. OPERACIÓN: ${body.operation_code || ''}`);
		...
		printer.println("2.- Niños a partir de 5 años deberán adquirir su boleto.  Los menores deben viajar acompañados de padre o madre presentando su DNI y/o Partida de Nacimiento original.");
...
app.post('/ticket/invoice/20614709562', (req, res) => { // NUEVO ILUCAN - 20614709562
    console.log(req.body);
    let body = req.body;
    if (typeof (body) === "string") {
        body = JSON.parse(body);
...
app.post('/ticket/invoice/20614485168', (req, res) => { // LINEBUS - 20614485168
    console.log(req.body);
    let body = req.body;
    if (typeof (body) === "string") {
        body = JSON.parse(body);
...
app.post('/ticket/invoice/20604329036', (req, res) => { // VIA EN BUS - 20604329036
    console.log(req.body);
    let body = req.body;
    if (typeof (body) === "string") {
        body = JSON.parse(body);
    ...
    printer.printImage(logo).then(function (done) {
        ...
        printer.bold(true)
        printer.println(`${body.enterprise_name}`);
        printer.println(`VIA EN BUS TRAVEL EMPRESA INDIVIDUAL DE RESPONSABILIDAD LIMITADA`);
        printer.bold(false)
        printer.println(`${body.enterprise_address}`)
        printer.println(`PUNTO DE EMISIÓN: ${body.seller_agency}`)
...
app.post('/ticket/invoice/20491796856', (req, res) => { //EZAPE - 20491796856
    console.log(req.body);
    let body = req.body;
    if (typeof (body) === "string") {
        body = JSON.parse(body);
    ...
    printer.printImage(logo).then(function (done) {
        ...
        printer.bold(true)
        printer.println(`${body.enterprise_name}`);
        printer.println(`TRANSPORTES EZAPE-LAJAS EIRL`);
        printer.bold(false)
        printer.println(`${body.enterprise_address}`)
        printer.println(`PUNTO DE EMISIÓN: ${body.seller_agency}`)
